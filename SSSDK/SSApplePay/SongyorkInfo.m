...
- (BOOL)paramterIsNilOrNotStringShowToViewController:(UIViewController *)viewController{
    //是否是nil
    ...
    for (int i = 0; i < count; i++) {
        ...
        if ([self respondsToSelector:getSeletor]) {
            ...
            if (returnValue == nil) {
                [SS_PublicTool showAlertToViewController:viewController alertControllerTitle:@"参数包含'nil'" alertControllerMessage:[NSString stringWithFormat:@"参数 : %@ 为nil", [NSString stringWithUTF8String:propertyName]] alertCancelTitle:@"知道了" alertReportTitle:nil cancelHandler:nil reportHandler:nil completion:nil];
                return NO;
            }
    ...
    for (id key in paramterDict) {
        if (![[paramterDict valueForKey:key] isKindOfClass:[NSString class]]) {
            SYLog(@"%@", [NSString stringWithFormat:@"参数 %@ : %@ 不为字符串", key, paramterDict[key]]);
            [SS_PublicTool showAlertToViewController:viewController alertControllerTitle:@"参数不为字符串" alertControllerMessage:[NSString stringWithFormat:@"参数 : %@ 不为字符串", key] alertCancelTitle:@"知道了" alertReportTitle:nil cancelHandler:nil reportHandler:nil completion:nil];
            return NO;
        }
    }
