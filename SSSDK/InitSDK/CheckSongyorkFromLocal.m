...
/* *计数器 */
@property (nonatomic, assign) int SS_count;
...
- (void)startScanningLocally{
    ...
    dispatch_source_set_event_handler(self.time, ^{
        ...
        //设置当执行五次是取消定时器
        weakSelf.SS_count++;
        if(weakSelf.SS_count == 5){
            dispatch_cancel(weakSelf.time);
        }
        
        NSDictionary *dict = [KeyChainWrapper load:SS_apple_dict_key];
        ...
        if (dict.count > 0) {
            for (NSNumber *key in dict) {
                ...
                dispatch_async(dispatch_get_main_queue(), ^{
                    ...
                    [[SS_SDKNetworkTool sharedSS_SDKNetworkTool] checkSYToServerWithReceiptInfo:[dict objectForKey:key] completion:^(BOOL isSuccess, id  _Nullable respones) {
                        if (isSuccess) {
