...
@interface SS_FloatWindow  ()

@property (nonatomic, strong)UIViewController *floatRootVC;

@property (nonatomic ,strong) UIButton *mainBtn;

@property (nonatomic, assign) BOOL isShowDetail;//是否展开悬浮窗

@property (nonatomic, assign) CGRect startFrame;//初始位置

@property (nonatomic, assign) NSInteger btnWidth;//btn宽度

@property (nonatomic ,strong) UIImageView *contentView;//点击btn展示详情界面

@property (nonatomic ,strong) NSDictionary *titleDict;//详情页标题

@property (nonatomic ,strong) NSArray *titleArr;//详情页图标

@property (nonatomic ,strong) NSArray *titileNameArr;//详情页图标


@property (nonatomic, assign) int btnTag;//详情每个按钮的tag值

@property (nonatomic ,strong) UIPanGestureRecognizer *moveGesture;//移动手势

@property (nonatomic ,strong) CMMotionManager *montionManager;//重力感应
...
@property(nonatomic,strong)NSString *backImgName; //悬浮按钮图片

@property (nonatomic ,strong) UIImageView *btnCentImg;

@property (nonatomic, assign) double orginX; //记录mianBtn位移前的X
...
@property (nonatomic, assign) int stayNum2;
...
//@property (nonatomic, strong) UIImageView *mainRedPointView;
...
//        _titleDict = titleDict;
        ...
        self.rootViewController = [[FloatViewNavController alloc] initWithRootViewController:self.floatRootVC];
...
//        self.layer.borderWidth = 1;
        ...
        if ([SS_SDKBasicInfo sharedSS_SDKBasicInfo].requestUrl.length < 1) {
            [SS_PublicTool mainWindowAddSubViewWindow:self];
        }
        ...
        [_mainBtn setImage:[SS_PublicTool getImageFromBundle:[SS_PublicTool getResourceBundle] withName:_backImgName withType:@"png"] forState:UIControlStateNormal];
...
//        Weak_Self;
        ...
        [_contentView setImage:[SS_PublicTool getImageFromBundle:[SS_PublicTool getResourceBundle] withName:@"bg_xf" withType:@"png"]];
...
//        [self accelerotionDataMethod];
        ...
        /**
         ...
         @return nil
         */
        [[SS_SDKNetworkTool sharedSS_SDKNetworkTool] getGameInfoCompletion:^(BOOL isSuccess, id response) {
            if (isSuccess) {
                [SS_SDKBasicInfo sharedSS_SDKBasicInfo].bbsID = response[@"bbs_fid"];
                [SS_SDKBasicInfo sharedSS_SDKBasicInfo].gameName = response[@"app_name"];
            }else{
                [SS_SDKBasicInfo sharedSS_SDKBasicInfo].bbsID = @"0";
            }
...
//        UIImageView *img = [[UIImageView alloc] init];
        ...
        [btn setImage:[SS_PublicTool getImageFromBundle:[SS_PublicTool getResourceBundle] withName:key withType:@"png"] forState:UIControlStateNormal];
//        btn.centerY = _contentView.centerY;
        btn.tag = _btnTag;
        
        // 则默认image在左，title在右
        // 改成image在上，title在下
        btn.titleEdgeInsets = UIEdgeInsetsMake(-10.0 , -[SS_PublicTool getImageFromBundle:[SS_PublicTool getResourceBundle] withName:key withType:@"png"].size.width, -30.0, 0.0);
...
//                btn.titleLabel.hidden = YES;
        ...
        [lineImageView setImage:[SS_PublicTool getImageFromBundle:[SS_PublicTool getResourceBundle] withName:@"xian" withType:@"png"]];
        ...
        if (_btnTag == 105) {
            ...
            if (_titleArr.count < 6) {
                ...
                btn.titleEdgeInsets = UIEdgeInsetsMake(-5.0 , -[SS_PublicTool getImageFromBundle:[SS_PublicTool getResourceBundle] withName:key withType:@"png"].size.width -5.0, -25.0, -5.0);
...
-(void)hideLeftFloat
{
    ...
    [UIView animateWithDuration:0.5f animations:^{
        ...
        //            [_mainBtn setImage:[SS_PublicTool getImageFromBundle:[SS_PublicTool getResourceBundle] withName:@"left_float" withType:@"png"] forState:UIControlStateNormal];
...
//        _mainBtn.frame = CGRectMake(0, 0, 20, 50);
        //            [_mainBtn setImage:[SS_PublicTool getImageFromBundle:[SS_PublicTool getResourceBundle] withName:@"right_float" withType:@"png"] forState:UIControlStateNormal];
...
//        [self timeForHiddenLOrR:0];
    ...
    if (_isShowDetail)
    {
        ...
        [_mainBtn setImage:[SS_PublicTool getImageFromBundle:[SS_PublicTool getResourceBundle] withName:_backImgName withType:@"png"] forState:UIControlStateNormal];
        [[SS_SDKNetworkTool sharedSS_SDKNetworkTool] checkGameNoticeCompletion:^(BOOL isSuccess, id respones) {
            if (isSuccess) {
...
/*
- (void)timeForHiddenLOrR:(int)lOrR{
    Weak_Self;
    self.stayNum2 = 0;
    //获得队列
    dispatch_queue_t queue = dispatch_get_global_queue(0, 0);
    ...
    dispatch_source_set_event_handler(self.time, ^{
        ...
        _stayNum2++;
        if (_stayNum2 == 5) {
            if (_mainBtnX < Screen_Width) {
                if (_mainBtnY != _mainBtn.y) {
                    
                    [weakSelf stopTimeFotLOR:lOrR];
 
                
                }
            }else{
                if (_mainBtnX != _mainBtn.x) {
                    [weakSelf stopTimeFotLOR:lOrR];
                }else{
                    dispatch_source_cancel(_time);
...
- (UIImageView *)redPointView{
    if (!_redPointView) {
        ...
        [_redPointView setImage:[SS_PublicTool getImageFromBundle:[SS_PublicTool getResourceBundle] withName:@"Red_point" withType:@"png"]];
        self.redPointView.hidden = YES;
    }
