...
- (void)getBasiceInfoForGame{
    ...
    [[SS_SDKNetworkTool sharedSS_SDKNetworkTool] getGameInfoCompletion:^(BOOL isSuccess, id response) {
        ...
        if (isSuccess) {
            ...
            [SS_SDKBasicInfo sharedSS_SDKBasicInfo].gameName = response[@"data"][@"app_name"];
            weakSelf.gameNameString = [SS_SDKBasicInfo sharedSS_SDKBasicInfo].gameName;
    ...
    [[PHPhotoLibrary sharedPhotoLibrary] performChanges:^{
        ...
        if (error) {
            ...
            if ([[NSUserDefaults standardUserDefaults]boolForKey:@"isAllow"]) {
                ...
                NSString *gameName = [NSString stringWithFormat:@"%@想访问您的相册,是否允许?(设置完毕请再次点击'保存')", [SS_SDKBasicInfo sharedSS_SDKBasicInfo].gameName];
                [SS_PublicTool showAlertToViewController:self.vc alertControllerTitle:@"提示" alertControllerMessage:gameName alertCancelTitle:@"取消" alertReportTitle:@"去设置" cancelHandler:^(UIAlertAction * _Nonnull action) {
...
- (UILabel *)userNameLab{
    if (!_userNameLab) {
        ...
        _userNameLab.text = [KeyChainWrapper load:SS_UserName_Fast];
...
- (UILabel *)passwordLab{
    if (!_passwordLab) {
        ...
        _passwordLab.text = [KeyChainWrapper load:SS_Password_Fast];
...
/*
 ...
 if ([[SS_SDKBasicInfo sharedSS_SDKBasicInfo] directionNumber] == 0) {
 if ([[SS_SDKBasicInfo sharedSS_SDKBasicInfo].deviceModel isEqualToString:@"iPhone_5S"] || [[SS_SDKBasicInfo sharedSS_SDKBasicInfo].deviceModel isEqualToString:@"iPhone_5C"] || [[SS_SDKBasicInfo sharedSS_SDKBasicInfo].deviceModel isEqualToString:@"iPhone_5"]){
 self.frame = CGRectMake(0, 0, 300, 280);
