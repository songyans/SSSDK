...
@interface SS_AddIdentityInfo()<UITextFieldDelegate>
{
    CGSize _noticeSize;
...
@implementation SS_AddIdentityInfo
...
- (void)submitClick{
    ...
    if (self.nameTextFiled.text.length < 1 || self.identityTextFiled.text.length < 15) {
        [SS_PublicTool showHUDWithViewController:_viewController Text:@"请按规定填写身份信息"];
        return;
    }else{
        //a-zA-Z0-9
        NSString *regex = @"^([\u4E00-\u9FA5]+)$";
        NSPredicate *pred = [NSPredicate predicateWithFormat:@"SELF MATCHES %@", regex];
        BOOL isEmpty = [SS_PublicTool isEmpty:self.nameTextFiled.text];
    ...
    Weak_Self;
    [[SS_SDKNetworkTool sharedSS_SDKNetworkTool] verifyIdcardWithName:self.nameTextFiled.text idCard:self.identityTextFiled.text completion:^(BOOL isSuccess, id  _Nullable respones) {
        if (isSuccess) {
           [weakSelf getAddIdentityInfoViewClickStates:ViewIsClickSubmit];
        }else{
           [SS_PublicTool showHUDWithViewController:_viewController Text:respones[@"msg"]];
        }
    } failure:^(NSError * _Nullable error) {
        [SS_PublicTool showHUDWithViewController:_viewController Text:@"网络异常"];
    }];
...
//    float passTextY = self.identityBorderView.height + self.identityBorderView.y;
//    float keyY = self.height - height;
//    if (self.identityTextFiled.isFirstResponder) {
//        if (passTextY >= keyY) {
//            SYLog(@"输入框被遮挡");
//
//            if (_idY != self.identityBorderView.y) {
//                self.identityBorderView.y = Screen_Height - (height + self.identityBorderView.height) ;
//                _idY = self.identityBorderView.y;
//            }
//
//
//        }
...
- (UIButton *)closeBtn{
    if (!_closeBtn) {
        ...
        [_closeBtn setImage:[SS_PublicTool getImageFromBundle:[SS_PublicTool getResourceBundle] withName:@"close" withType:@"png"] forState:UIControlStateNormal];
    }
    return _closeBtn;
}
