...
@interface SS_RegisterMobilUserViewController ()<UITextFieldDelegate>
...
@implementation SS_RegisterMobilUserViewController
...
- (void)nextStepClick{
    ...
    Weak_Self;
    if (![SS_PublicTool isValidateTel:self.phoneNumTextField.text]) {
        [SS_PublicTool showHUDWithViewController:self Text:@"请输入正确的手机号码"];
        return;
    }
    NSInteger userType;
    if ([SS_PublicTool isValidateTel:self.phoneNumTextField.text]) {
...
//        [PublicTool showHUDWithViewController:self Text:@"格式错误"];
    ...
    [[SS_SDKNetworkTool sharedSS_SDKNetworkTool] checkIfTheUserExistsWithUserName:self.phoneNumTextField.text userType:userType completion:^(BOOL isSuccess, id  _Nullable response) {
...
//        code = -1;
        switch (code) {
            case 0:
            {
                SS_LoginForPhoneViewController *phoneVC = [[SS_LoginForPhoneViewController alloc] init];
                phoneVC.isPush = YES;
            ...
            case -2:
            {
                [SS_PublicTool showHUDWithViewController:weakSelf Text:@"手机号已绑定用户"];
            }
                break;
