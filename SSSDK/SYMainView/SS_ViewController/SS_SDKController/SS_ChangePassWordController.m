...
@interface SS_ChangePassWordController ()<UITextFieldDelegate>

@property (nonatomic ,strong) SS_BGView  *BGView;//背景
...
@implementation SS_ChangePassWordController
/*
 * 注销键盘的通知事件
 */
- (void)dealloc
...
- (void)sureClick:(id)sender{
    ...
    if (self.passwordTextField.text.length < 6) {
        [SS_PublicTool showHUDWithViewController:self Text:@"密码最少6个字符"];
        return;
    }
    /*正则表达式判断是否有中文(屌不屌)*/
    NSString *regex = @"^([\u4E00-\u9FA5]+)$";
    NSPredicate *pred = [NSPredicate predicateWithFormat:@"SELF MATCHES %@", regex];
    BOOL isEmpty = [SS_PublicTool isEmpty:self.passwordTextField.text];
    ...
    if([pred evaluateWithObject:self.passwordTextField.text] || isEmpty) {
        [SS_PublicTool showHUDWithViewController:self Text:@"请按正确格式填写密码"];
    ...
    [[SS_SDKNetworkTool sharedSS_SDKNetworkTool] changePassword:self.passwordTextField.text phoneNum:self.phoneNum code:self.code completion:^(BOOL isSuccess, id respones) {
        if (isSuccess) {
            SYLog(@"-------respones:%@", respones);
            [SS_PublicTool showHUDWithViewController:weakSelf Text:@"修改成功"];
            BOOL firstOpen = [SS_PublicTool getCurrenFirstOpen];
            if (firstOpen) {
...
- (UILabel *)tipsLab{
    if (!_tipsLab) {
        ...
        _tipsLab = [SS_PublicTool changeTextColor:[UIColor colorWithRed:0.18 green:0.55 blue:0.91 alpha:1.00] ChangeTitle:self.account Titile:_tipsLab.text ToLabel:_tipsLab];
...
- (UIImageView *)passwordImgView{
    ...
    if (!_passwordImgView) {
        ...
        [_passwordImgView setImage:[SS_PublicTool getImageFromBundle:[SS_PublicTool getResourceBundle] withName:@"tb_pwd" withType:@"png"]];
