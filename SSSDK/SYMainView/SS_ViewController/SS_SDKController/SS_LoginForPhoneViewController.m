...
@interface SS_LoginForPhoneViewController ()<UITextFieldDelegate,UITableViewDelegate,UITableViewDataSource,WKNavigationDelegate,WKUIDelegate,WKScriptMessageHandler,SYUserCellDelegate>
...
@implementation SS_LoginForPhoneViewController
...
- (void)viewWillAppear:(BOOL)animated{
    ...
    NSDictionary *passDic = [KeyChainWrapper load:SSPasswordKey];
    ...
    if (nameArr.count > 0) {
        for (NSString *key in nameArr) {
            if ([SS_PublicTool isValidateTel:key]) {
...
- (void)loginClick:(id)sender{
    ...
    if (user.length < 6 || user == nil) {
        ...
        BOOL isEmpty = [SS_PublicTool isEmpty:user];
        if(isEmpty) {
        ...
        if (![SS_PublicTool isValidateTel:user]) {
            userType = 0;
        }else{
            userType = 1;
        }
    ...
    if ([SS_PublicTool isValidateTel:user]) {
        ...
        [[SS_SDKNetworkTool sharedSS_SDKNetworkTool] checkIfTheUserExistsWithUserName:self.userTextFiled.text userType:userType completion:^(BOOL isSuccess, id  _Nullable response) {
...
#pragma mark ----------------------------------------------强制进入注册流程
            ...
            switch (code) {
                ...
                case -1:
                {
                    ...
                    SS_GetCodeForMessageViewController *regPhoneVC = [[SS_GetCodeForMessageViewController alloc] init];
...
- (void)userLoginWithUsername:(NSString *)username{
    ...
    [[SS_SDKNetworkTool sharedSS_SDKNetworkTool] checkExpiredForToken:token userName:user completion:^(BOOL isSuccess, id  _Nullable resp) {
...
#pragma mark ----------------------------------------------强制进入身份验证页
            ...
            if ([SS_PublicTool isValidateTel:user]) {
                ...
                SS_GetCodeForMessageViewController *getVC = [[SS_GetCodeForMessageViewController alloc] init];
                ...
                NSMutableDictionary *passwordDic = [KeyChainWrapper load:SSPasswordKey];
                NSString *password = [passwordDic valueForKey:user];
                if (password.length < 1) {
                    NSString *touristUser = [KeyChainWrapper load:SS_UserName_Fast];
                    if ([user isEqualToString:touristUser]) {
                        password = [KeyChainWrapper load:SS_Password_Fast];
                    }else{
                        password = @"";
                    }
                }
                [[SS_SDKNetworkTool sharedSS_SDKNetworkTool] loginWithUserName:user password:password completion:^(BOOL isSuccess, id  _Nullable respones) {
                    if (isSuccess) {
                        NSDictionary *loginDic = resp[@"data"];
                        SYLog(@"------respones:%@", resp);
...
- (void)makeTheViewForLogin{
    ...
    self.bgView.CleanDeviceBlock = ^(BOOL isCleanSuccess) {
        if (isCleanSuccess) {
            [SS_PublicTool showAlertToViewController:weakSelf alertControllerTitle:@"清除成功" alertControllerMessage:[NSString stringWithFormat:@"设备号已重置,请彻底关闭应用重新打开"] alertCancelTitle:@"好的" alertReportTitle:nil cancelHandler:nil reportHandler:nil completion:nil];
        }else{
            [SS_PublicTool showAlertToViewController:weakSelf alertControllerTitle:@"清除失败" alertControllerMessage:[NSString stringWithFormat:@"设备号已清除失败,请重试"] alertCancelTitle:@"好的" alertReportTitle:nil cancelHandler:nil reportHandler:nil completion:nil];
        }
    };
    [self createUI];
    ...
    if (self.fastPassStr.length > 1) {
        if ([SS_SDKBasicInfo sharedSS_SDKBasicInfo].needAuto) {
            if ([SS_PublicTool isValidateTel:self.fastUserStr]) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    SS_UserModel *model = [SS_UserModel new];
...
- (void)cell:(UITableViewCell *)cell deleteUsernameInfoIsDone:(BOOL)isDone deleteUsername:(NSString *)username{
    if (isDone) {
        NSMutableArray *nameArr = [KeyChainWrapper load:SSUsernameKey];
        /*
         * 游客登录账号密码
         */
        NSString *fastName = [KeyChainWrapper load:SS_UserName_Fast];
        ...
        if (fastPass.length > 0) {
            ...
            if (![nameArr containsObject:fastName]) {
                ...
                //保存进字典
                NSMutableDictionary *dict = [KeyChainWrapper load:SSPasswordKey];
...
- (UIButton *)findBtn{
    if (!_findBtn) {
        ...
        //            [_findBtn setBackgroundImage:[SS_PublicTool getImageFromBundle:[SS_PublicTool getResourceBundle] withName:@"anv_02_1" withType:@"png"] forState:UIControlStateNormal];
        //            [_findBtn setBackgroundImage:[SS_PublicTool getImageFromBundle:[SS_PublicTool getResourceBundle] withName:@"anv_02_2" withType:@"png"] forState:UIControlStateHighlighted];
...
- (UIButton *)loginBtn{
    if (!_loginBtn) {
        ...
        //            [_loginBtn setBackgroundImage:[SS_PublicTool getImageFromBundle:[SS_PublicTool getResourceBundle] withName:@"an1_01" withType:@"png"] forState:UIControlStateNormal];
        //            [_loginBtn setBackgroundImage:[SS_PublicTool getImageFromBundle:[SS_PublicTool getResourceBundle] withName:@"an1_02" withType:@"png"] forState:UIControlStateHighlighted];
        //        [_loginBtn setBackgroundImage:[SS_PublicTool getImageFromBundle:[SS_PublicTool getResourceBundle] withName:@"an1_1_01" withType:@"png"] forState:UIControlStateNormal];
        //        [_loginBtn setBackgroundImage:[SS_PublicTool getImageFromBundle:[SS_PublicTool getResourceBundle] withName:@"an1_2_02" withType:@"png"] forState:UIControlStateHighlighted];
...
- (SS_BGView *)bgView{
    if (!_bgView) {
        _bgView = [[SS_BGView alloc] initWithShowImage:NO showBGView:NO showCleanView:YES];
        _bgView.vc = self;
...
- (UIImageView *)logoImgView{
    if (!_logoImgView) {
        ...
        //            [_logoImgView setImage:[SS_PublicTool getImageFromBundle:[SS_PublicTool getResourceBundle] withName:@"Log" withType:@"png"]];
