...
@interface SS_UserManagerController ()
...
@implementation SS_UserManagerController
...
- (void)craeteUI{
    ...
    if (![[SS_SDKBasicInfo sharedSS_SDKBasicInfo].loginUser isEqualToString:[SS_SDKBasicInfo sharedSS_SDKBasicInfo].bindPhoneNumber]){
...
- (void)modify:(id)sender{
    ...
    SS_ModifyController *mVC = [[SS_ModifyController alloc] init];
...
- (void)bind:(id)sender{
    if ([SS_SDKBasicInfo sharedSS_SDKBasicInfo].isBindPhone) {
        SYLog(@"-----------改绑手机");

        [[SS_SDKNetworkTool sharedSS_SDKNetworkTool] changeBindMobilPhone];
    }else{
        Weak_Self;
        SYLog(@"-----------绑定手机");

        SS_BindMobileViewController *bindVC = [[SS_BindMobileViewController alloc] init];
        bindVC.HudHiddenBlock = ^(BOOL isShow) {
            weakSelf.showBHud = isShow;
            [SS_SDKBasicInfo sharedSS_SDKBasicInfo].isBindPhone = isShow;
...
- (void)signOutClick:(id)sender{
    ...
    [SS_PublicTool showAlertToViewController:self alertControllerTitle:@"提示" alertControllerMessage:@"是否退出游戏" alertCancelTitle:@"取消" alertReportTitle:@"是的" cancelHandler:nil reportHandler:^(UIAlertAction * _Nonnull action) {
...
- (void)SignOutGame{
    ...
    [center postNotificationName:SS_SignOutGame object:@"isOut" userInfo:dic];
