...
- (void)settingAutoLogin:(id)param messageName:(NSString *)name{
    ...
    if (isAuto) {
        [SS_PublicTool ifNeedAutoLogin:isAuto];
        NSString *userName = [SS_SDKBasicInfo sharedSS_SDKBasicInfo].fastUserName;
        NSString *userPass = userDict[userName];
        NSDictionary *userAutoDic = @{
            userName    :   userPass,
            };
        [KeyChainWrapper save:SS_UserLogin_Auto data:userAutoDic];
...
//        NSMutableDictionary *userDict = [KeyChainWrapper load:SSPasswordKey];
        ...
        [KeyChainWrapper save:SS_UserLogin_Auto data:usetAutoDic];
    }
}
