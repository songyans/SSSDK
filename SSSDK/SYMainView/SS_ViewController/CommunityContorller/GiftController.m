...
- (void)getGiftCValueWithMessageName:(NSString *)name param:(NSString *)param{
    ...
    if (pasteboard.string.length > 1) {
        [SS_PublicTool showAlertToViewController:self alertControllerTitle:@"复制成功" alertControllerMessage:nil alertCancelTitle:@"好的" alertReportTitle:nil cancelHandler:^(UIAlertAction * _Nonnull action) {
            
        } reportHandler:nil completion:^{
            
        }];
    }else{
        [SS_PublicTool showAlertToViewController:self alertControllerTitle:@"复制失败" alertControllerMessage:nil alertCancelTitle:@"好的" alertReportTitle:nil cancelHandler:^(UIAlertAction * _Nonnull action) {
...
- (void)doShareWithMessageName:(NSString *)name param:(NSString *)param{
    ...
    [SS_SDKBasicInfo sharedSS_SDKBasicInfo].giftId = param;
...
//    [center postNotificationName:SS_ShareGiftInfo object:@"shareInfo" userInfo:dic];
}

- (void)ifShareIsSuccess{
    SYLog(@"分享成功-------回调---------前端");
    NSString *jsString = [NSString stringWithFormat:@"shareCallback('%@','%@')", @"1", [SS_SDKBasicInfo sharedSS_SDKBasicInfo].giftId];
...
//    SYLog(@"--------------param:%@", param);
    if (param[@"ios"]) {
        ...
        jsString = [NSString stringWithFormat:@"getiOSToken('%@','%@')", sign, [SS_SDKBasicInfo sharedSS_SDKBasicInfo].sdkToken];
...
// 回到应用判断[SS_SDKBasicInfo sharedSS_SDKBasicInfo].isShareSuccess是否是yes
- (void)applicationDidBecomeActive:(UIApplication *)application {
    SYLog(@"回来主进程");
    if ([SS_SDKBasicInfo sharedSS_SDKBasicInfo].isShareSuccess) {
        [self ifShareIsSuccess];
        [SS_SDKBasicInfo sharedSS_SDKBasicInfo].isShareSuccess = NO;
    }
