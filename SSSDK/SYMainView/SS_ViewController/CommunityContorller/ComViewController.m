...
//    [self.webView addObserver:self forKeyPath:@"estimatedProgress" options:NSKeyValueObservingOptionNew context:nil];
    ...
    /*
    [[SS_SDKNetworkTool sharedSS_SDKNetworkTool] testInfoToAnyParamCompletion:^(BOOL isSuccess, id  _Nullable respones) {
        if (isSuccess) {
    ...
    [coordinator animateAlongsideTransition:^(id<UIViewControllerTransitionCoordinatorContext> context)
     ...
     } completion:^(id<UIViewControllerTransitionCoordinatorContext> context)
     {
         ...
         if (_duration == UIDeviceOrientationLandscapeLeft || _duration == UIDeviceOrientationLandscapeRight) {
             _buildImgView.image = [SS_PublicTool getImageFromBundle:[SS_PublicTool getResourceBundle] withName:@"build2" withType:@"jpg"];
             
         }else if (_duration == UIDeviceOrientationPortrait){
             _buildImgView.image = [SS_PublicTool getImageFromBundle:[SS_PublicTool getResourceBundle] withName:@"build1" withType:@"jpg"];
         }
...
- (void)showThePassCodeJS:(NSString *)passCodeJS{
    ...
    [self.webView evaluateJavaScript:passCodeJS completionHandler:^(id _Nullable result, NSError * _Nullable error) {
        ...
        if ([self.requestUrl isEqualToString:@""]) {
            [SS_SDKBasicInfo sharedSS_SDKBasicInfo].bbsID = @"2";
            if ([[SS_SDKBasicInfo sharedSS_SDKBasicInfo].bbsID intValue] == 0) {
//                self.requestUrl = @"https://bbs.shangshiwl.com/forum.php?forumlist=1&mobile=2";
                self.requestUrl = [NSString stringWithFormat:@"%@/forum.php?mod=forumdisplay&mobile=2", SS_Com_Html];

            }else{
//                weakSelf.requestUrl = [NSString stringWithFormat:@"https://bbs.shangshiwl.com/forum.php?mod=forumdisplay&fid=%@&mobile=2", [SS_SDKBasicInfo sharedSS_SDKBasicInfo].bbsID];
                self.requestUrl = [NSString stringWithFormat:@"%@/forum.php?mod=forumdisplay&fid=%@&mobile=2", SS_Com_Html, [SS_SDKBasicInfo sharedSS_SDKBasicInfo].bbsID];
            }
...
- (void)keepNetWoking{
    ...
    dispatch_source_set_event_handler(self.time, ^{
        if ([SS_SDKBasicInfo sharedSS_SDKBasicInfo].isNetWorking) {
...
- (void)loadDateToWebView{
    ...
    if ([nameArr containsObject:[SS_SDKBasicInfo sharedSS_SDKBasicInfo].loginUser]) {
        passString = [passD valueForKey:[SS_SDKBasicInfo sharedSS_SDKBasicInfo].loginUser];
        
    }
    
    
    NSDictionary *param = @{
                            @"username" :  [SS_SDKBasicInfo sharedSS_SDKBasicInfo].loginUser,
                            @"password" :  passString,
                            };
    
    NSURL *url = [NSURL URLWithString: SS_COM_Test];
    
    NSString *body = [NSString stringWithFormat: @"username=%@&password=%@&sdk_code=%@",param[@"username"],param[@"password"], [SS_PublicTool makeSignStringWithParams:param key:SS_BBS_KEY]];
    ...
    [self.webView evaluateJavaScript:javascript completionHandler:^(id _Nullable result, NSError * _Nullable error) {
        ...
        if (error) {
            if ([weakSelf.requestUrl isEqualToString:@"https://www.shangshiwl.com/wap/user/index"]) {
                [SS_SDKBasicInfo sharedSS_SDKBasicInfo].bbsID = @"47";
                if ([[SS_SDKBasicInfo sharedSS_SDKBasicInfo].bbsID intValue] == 0) {
