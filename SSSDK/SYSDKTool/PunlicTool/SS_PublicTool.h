...
@interface SS_PublicTool : NSObject

//@property (nonatomic, copy) NSString *code;//接受注册或者登录后返回的参数,做保存

//@property (nonatomic, copy) NSString *token;//接受注册或登录后返回的参数,做保存
...
/**
 ...
 @param bundle getResourceBundle返回累心
 @param name 图片名字
...
+ (UIButton *_Nonnull)createBtnWithButton:(UIButton *_Nullable)btn
                          ...
                          backgroundColor:(UIColor *_Nullable)backgroundColor
                            ...
                            highlightTitle:(NSString *_Nullable)highlightTitle
 ...
 */
+ (void)createFloatWindowIntoGameCompletion:(void(^_Nullable)(void))completion;
