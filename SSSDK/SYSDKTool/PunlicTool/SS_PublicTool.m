    ...
    if ([dict isKindOfClass:[NSDictionary class]] || [dict isKindOfClass:[NSMutableDictionary class]]) {
        ...
        for (id str in sortArray)
        {
            ...
            if ([[dict objectForKey:str] isKindOfClass:[NSString class]] && isEncode) {
                urlEncodedStr = [SS_PublicTool encodeString:[dict objectForKey:str]];
            }
            else{
...
+ (BOOL)isValidateTel:(NSString *_Nullable)tel
{
    ...
    NSString *patternTel = [SS_SDKBasicInfo sharedSS_SDKBasicInfo].regexModel.phone;
...
{
    return [SS_PublicTool toMD5:[NSString stringWithFormat:@"%@%@",password,[SS_PublicTool toMD5:password]]];
}
...
+ (void)fastLoginWithSecretCode:(NSString *_Nullable)secret{
    [[NSUserDefaults standardUserDefaults] setObject:secret forKey:SS_fastLogin];
    [[NSUserDefaults standardUserDefaults] synchronize];
}

+ (NSString *_Nullable)getSecretCodeForFastLogin{
    NSString *secret = [[NSUserDefaults standardUserDefaults] objectForKey:SS_fastLogin];
    return secret;
}
...
+ (UIButton *)createBtnWithButton:(UIButton *)btn buttonType:(UIButtonType)buttonType frame:(CGRect)frame backgroundColor:(UIColor *)backgroundColor image:(UIImage *)image normalTitle:(NSString *)normalTitle selectedTitle:(NSString *)selectedTitle highlightTitle:(NSString *)highlightTitle textAlignment:(NSTextAlignment)textAlignment selected:(BOOL)selected titleNormalColor:(UIColor *)titleNormalColor titleSelectedColor:(UIColor *)titleSelectedColor titleHighlightedColor:(UIColor *)titleHighlightedColor{
    ...
    if (!highlightTitle || highlightTitle.length < 1) {
        highlightTitle = normalTitle;
    }
...
+ (NSDictionary *)signKeyMustStringFromDictionary:(NSDictionary *)dictionary{
    ...
    NSString *signStr = [SS_PublicTool makeSignStringWithParams:parameterDictionary];
    SYLog(@"---sign:%@", signStr);
...
+ (void)createFloatWindowIntoGameCompletion:(void (^)(void))completion{
...
+ (BOOL)isIphone_X{
    BOOL isIphone_x = NO;
    if ([[SS_SDKBasicInfo sharedSS_SDKBasicInfo].device_Model isEqualToString:@"iPhone_X"]) {
        isIphone_x = YES;
    }
    return isIphone_x;
}
